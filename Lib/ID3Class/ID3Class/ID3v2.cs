...
namespace ID3
{
    public class ID3v2
    {
        ...
        public void Load()
        {
            using (FileStreamEx ID3File = new FileStreamEx(_FilePath, FileMode.Open, FileAccess.Read))
            {
                if (!ID3File.HaveID3v2()) // If file don't contain ID3v2 exit function
                {
                    _HaveTag = false;
                    _ver = new Version("2.3.0"); // HERE
                    ID3File.Close();
                    return;
                }
        ...
        /// <summary>
        /// NEU VON JUS (24.12.2008): 
        /// Set text of specific TextFrame with language
        /// Language wird auf "" gesetzt
        /// </summary>
        /// <param name="FrameID">FrameID</param>
        /// <param name="Text">Text to set</param>
        /// <param name="TextEncoding">Enxoding of text</param>
        /// <param name="ver">minor version of ID3v2</param>
        public void SetTextWithLanguageFrame(string FrameID, string Text)
        {
            if (!FramesInfo.IsValidFrameID(FrameID))
                return;

            for (int i = 0; i < _TextWithLangFrames.Count - 1; i++)
            {
                if (_TextWithLangFrames.Items[i].FrameID == FrameID)
                {
                    _TextWithLangFrames.RemoveAt(i);
                    break;
                }
            }
            ...
            if (Text != null)
            {
                _TextWithLangFrames.Add(new TextWithLanguageFrame(FrameID, new FrameFlags(),
                    Text, "", (Frame.IsAscii(Text) ? TextEncodings.Ascii : _DefaultUnicodeEncoding),
                    ""));
            }
        }
        ...
        private void ReadFrames(FileStreamEx Data, int Length)
        {
            ...
            while (Length > 10)
            {
                ...
                uint fl = Data.ReadUInt(FrameIDLen);
                FrameLength = Convert.ToInt32(fl);
                if (FrameIDLen == 4)
        ...
        private bool AddFrame(FileStreamEx Data, string FrameID, int Length, FrameFlags Flags)
        {
            ...
            switch (FrameID)
            {
                ...
                case "LINK":
                    ...
                    break;
                case "APIC":
                    AttachedPictureFrame TempAttachedPictureFrame = new AttachedPictureFrame(FrameID, Flags, Data, Length, VersionInfo);
                    if (TempAttachedPictureFrame.IsReadableFrame)
                    { _AttachedPictureFrames.ReallyAdd(TempAttachedPictureFrame); return true; }
                    else
