...
namespace System.IO
{
    internal class FileStreamEx : FileStream
    {
        ...
        public FileStreamEx(string path, FileMode mode, FileAccess access)
        ...
        public string ReadText(int MaxLength, TextEncodings TEncoding, ref int ReadedLength, bool DetectEncoding)
        {
            ...
            while (MaxLength > 0)
            {
                ...
                else // if Buf == 0
                {
                    ...
                    {
                        byte Temp = ReadByte();
                        if (Buf == 0 && Temp == 0) // changed
                        {
                            if (TEncoding == TextEncodings.UTF_16)
                            {//Hack here was something wrong. Inserted: MStream.WriteByte and MaxLength--
                                MStream.WriteByte(Buf);
                                MaxLength--;
                                break;
                            }
