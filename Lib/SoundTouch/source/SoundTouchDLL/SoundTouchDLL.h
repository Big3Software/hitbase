...
SOUNDTOUCHDLL_API float __stdcall soundtouch_detectBPM(int numChannels, int sampleRate, float samples[], int samplesLength);
...
namespace Big3
{
	namespace Hitbase
	{
		...
		{
			public ref class SoundTouchNet
			{
			public:
				// Detect BPM rate of inFile and adjust tempo setting accordingly if necessary
				float DetectBPM(int numChannels, int sampleRate, array<float>^ samples, int samplesLength)
				{
					float bpmValue;
					int nChannels;
					BPMDetect bpm(numChannels, sampleRate);

					nChannels = numChannels;

					float* nativeSamples = new float[samplesLength];
					for (int i=0;i<samplesLength;i++)
					{
						nativeSamples[i] = samples[i];
					}
					// Enter the new samples to the bpm analyzer class
					int numSamples = samplesLength / numChannels;
					bpm.inputSamples(nativeSamples, numSamples);
