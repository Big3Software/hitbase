...
namespace Big3.Hitbase.Miscellaneous
{
    ...
    public class SafeObservableCollection<T> : IList<T>, INotifyCollectionChanged
    {
        ...
        private void DoRemoveAt(int index)
        {
            ...
            if (CollectionChanged != null)
                CollectionChanged(this,
                    new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, this.collection[index], index));
            sync.ReleaseWriterLock();
    ...
    {
        public void AddItemFromThread(T item)
        {
            System.Windows.Application.Current.Dispatcher.Invoke((Action)(() => { Add(item); }));
        }

        public void InsertItemFromThread(int index, T item)
        {
            System.Windows.Application.Current.Dispatcher.Invoke((Action)(() => { Insert(index, item); }));
        }

        public void ClearFromThread()
        {
            System.Windows.Application.Current.Dispatcher.Invoke((Action)(() => { Clear(); }));
        }

        public void RemoveAtFromThread(int index)
        {
            System.Windows.Application.Current.Dispatcher.Invoke((Action)(() => { RemoveAt(index); }));
        }
