...
void CTwain::GetIdentity()
{
	...
	m_AppId.Version.Language = TWLG_GER;
	m_AppId.Version.Country = TWCY_GERMANY;
	strcpy (m_AppId.Version.Info, "1.0");
	m_AppId.ProtocolMajor = TWON_PROTOCOLMAJOR;
	m_AppId.ProtocolMinor = TWON_PROTOCOLMINOR;
	m_AppId.SupportedGroups = DG_IMAGE | DG_CONTROL;
	strcpy (m_AppId.Manufacturer, "Big 3 Software");
	strcpy (m_AppId.ProductFamily, "Generic");
	strcpy (m_AppId.ProductName, "Hitbase");
}
...
BOOL CTwain::SelectSource()
{
	...
	BOOL bTwainDriverfound;
	int nCount=0;

	// Wir wollen den Dialog nur dann haben, wenn es auch mehr als einen Twain Treiber gibt
	// Also zählen wir mal :-)
	bTwainDriverfound = CallTwainProc(&m_AppId,NULL,DG_CONTROL,DAT_IDENTITY,MSG_GETFIRST,&m_Source);
	while (bTwainDriverfound == TRUE)
	{
		nCount++;
		bTwainDriverfound = CallTwainProc(&m_AppId,NULL,DG_CONTROL,DAT_IDENTITY,MSG_GETNEXT,&m_Source);
	}
...
*/
BOOL CTwain::Acquire(int numImages, TWAINCALLBACKPROC TwainCallbackProc /* = NULL */, LPARAM lParam /* = 0 */)
{
	m_pTwainCallbackProc = TwainCallbackProc;
	m_lParam = lParam;
	m_nTransferedImages = 0;
...
void CTwain::TranslateMessage(TW_EVENT& twEvent)
{
	...
	// Cancel Twain-Dialog automatically after scanning number of images
//	if (m_nImageCount > 0 && m_nTransferedImages >= m_nImageCount)
//		ReleaseTwain();
}
...
//	if (m_nImageCount > 0 && m_nTransferedImages >= m_nImageCount)
	...
	//TODO!!!!!!!!!!!!!!!CPicture TwainPicture;
	//TODO!!!!!!!!!!!!!!!TwainPicture.LoadFromHandle ((HBITMAP)hBitmap);

	//TODO!!!!!!!!!!!!!!!(*m_pTwainCallbackProc)(&TwainPicture, m_lParam);
}
