...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	{
		// JUS 13.08.2003: Etwas Rand einstellen, sonst zu eng.
...
//		DrawTheText(pDC, (LPCTSTR)sTitle, &lpDIS->rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
		DrawTheText(pDC, (LPCTSTR)sTitle, &lpDIS->rcItem, &rect, m_bIsPressed, m_bIsDisabled);
	} // if
		...
		case ST_ALIGN_OVERLAP:
			{
				...
				// Center image vertically
//				rpImage->top += ((rpImage->Height() - (long)dwHeight)/2);
				rpImage->top = rBtn.bottom - dwHeight - m_ptImageOrg.y;
				break;
			}
	} // switch
...
void CButtonST::InitToolTip()
{
	if (m_ToolTip.m_hWnd == NULL)
	{
		...
		// JUS 14.05.2004: Damit Tooltip länger auf dem Bildschirm erscheint
		m_ToolTip.SetDelayTime(TTDT_AUTOPOP, 30000);
		// Enable multiline
		::SendMessage(m_ToolTip.m_hWnd, TTM_SETMAXTIPWIDTH, 0, 400);
