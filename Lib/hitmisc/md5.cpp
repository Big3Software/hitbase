...
CString CMD5::MD5String(char* szString)
{
...
CString CMD5::MD5File(const CString& szFilename)
{
	FILE* file;
	CMD5 alg;
	int nLen;
	const int nBufferSize = 8192;
	unsigned char chBuffer[nBufferSize];

	try
	{
		memset(chBuffer, 0, nBufferSize);

		if ((file = _wfopen (szFilename, L"rb")) != NULL)
		{
			while (nLen = fread (chBuffer, 1, nBufferSize, file))
